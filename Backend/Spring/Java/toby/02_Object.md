...
- 싱글톤을 다룰 때의 주의 사항
    - stateless를 최대한 유지해라. (파라미터, 로컬변수, 리턴을 활용하자)
        - 상태를 갖게 되는 순간 지옥
        - synchronized 키워드도 있지만 가능한 설계 자체를 상태가 없도록 유지하기 위해 노력하자.
    - 스프링 빈끼리 사용하는 경우에는 인스턴스 변수로 사용해도 괜찮다.
    - 읽기 전용 속성을 가진 정보도 인스턴스 변수로 사용해도 괜찮다.
        - 하지만 읽기 전용은 static final, final을 선언하는 게 더 좋다.
        
- 스코프
    - 스프링 빈이 생성되고 존재하고 적용되는 범위
    - 기본적으로 싱글톤이다.
    - 프로토타입, 리퀘스트, 세션 등의 스코프가 있다. (10장 참고)
    
### 1.7. 의존관계 주입(DI)
- 다시 보는 IoC
    - 객체를 생성하고 관계를 맺어주는 등의 작업
    - 스프링이 이 동작을 해준다.
    
- DI는?
    - IoC를 구현하는 스프링의 방법론을 좀 더 명확하게 드러낸 단어
    - `의존관계 주입` 혹은 `의존관계 설정`이 의도를 가장 명확하게 드러내는 용어다.
    
- 의존관계?
    - 의존하다: 의존하는 대상이 변화하면 사용하는 대상도 영향을 받는다는 의미
        - 현재 예제에서는 ConnectionMaker가 변하면 UserDao도 영향을 받는다.
    - 의존 관계에는 `방향성`이 필수다.
    
- 설계상의 의존과 런타임 의존
    - UserDao는 코드를 통해 ConnectionMaker에 의존하고 있다. => 설계상 의존
    - 하지만 실제로 애플리케이션이 동작하면 ConnectionMaker를 준수한 어떤 오브젝트에 의존하게 된다 => 런타임 의존
    - 여기서 런타임에 사용되는 실제 오브젝트를 `의존 오브젝트`라 한다.
    
- 의존관계 주입
    - 런타임 의존성을 갖는 구체적인 의존 오브젝트와 그것을 사용할 클라이언트 오브젝트 간에 런타임 시에 연결해주는 작업
    - 의존관계 주입의 조건
        - 설계상에서 `인터페이스에만 의존`하고 있어야 한다.
        - 런타임 시 의존 관계는 컨테이너나 팩토리 같은 `제3의 존재가 결정`한다.
        - 의존관계는 사용할 오브젝트에 대한 레퍼런스를 `외부에서 제공`한다.
    - 가장 중요한 건 제3의 존재가 있고, 이 제3의 존재가 의존관계를 결정한다는 것
    - 일련의 DI 과정에서 자연스럽게 사용할 오브젝트의 선택과 생성 제어권을 외부로 넘기고 수동적으로 주입받는 오브젝트를 사용하게 된다.
    - IoC의 개념과 아주 잘 맞는다.

- 의존관계 검색
    - 미리 정해진 스프링 bean을 여러 형태로 검색해서 DI하는 방식
    - DI와 거의 같다. 세부 방법이 살짝 다를 뿐이다.
    - 의존관계 검색은 내부에 스프링 API가 노출되기 때문에 관심사 분리에서 썩 좋은 코드는 아니다.
    - 하지만 의존관계 검색이 필수적일 때가 있다.
        - 테스트 코드나 main 코드의 경우 DI 받을 방법이 없기 때문에 한번은 `의존관계 검색`을 수행해야 한다.
    - 의존관계 주입 vs. 의존관계 검색
        - 의존관계 주입은 사용할 오브젝트와 주입되는 오브젝트가 `모두 스프링 빈`이어야만 한다.
        - 의존관계 검색은 클라이언트 코드 쪽이 굳이 스프링 빈일 필요가 없다.

- DI 활용
    - 기능 구현의 교환
        - 테스트용, 개발용, 검증용 DB 연결 설정 정보를 나누고 필요에 따라 변경하도록 구현할 수 있다.
        - 스프링 부트의 설정도 이런 식으로 동작한다.
    - 부가 기능 추가
        - 기존 빈에 새로운 기능을 추가하고 싶을 때 새로운 빈이 기존 빈을 감싸도록 응용할 수 있다.
        - 일종의 AOP 예제
    - 관심사의 분리를 통한 높은 응집도를 통해 위와 같은 예제들을 구현할 수 있다.
    - 스프링을 공부하는 건 DI를 어떻게 활용할지 공부하는 것이다. 이런걸 어떻게 만들어낼지 잘 고민하자.

### 1.8 XML을 이용한 설정
- DI의 방법 중 하나로 XML이 있지만, 현재 반드시 필요하진 않다. 필요할 때 보자
- ref, value 프로퍼티로 필요한 오브젝트나 값을 주입할 수 있다.
    - ref: 의존성 주입에 필요한 빈
    - value: 빈 생성에 필요한 값
- 값을 주입하는 이유
    - 오브젝트(빈)을 주입하는 이유는 구현 클래스를 다이나믹하게 바꾸면서 여러 이점을 얻기 위해서다.
    - 값을 주입하는 것은 이것과는 약간 다른 이유다.
        - 값을 주입하면 사용할 오브젝트 자체를 바꾸지 않고, 오브젝트의 특성을 바꿀 수 있다.
        - 값 객체는 `값 그 자체로 의미를 갖고`, `값 자체로 식별자`이기 때문이다.
        - 즉, 값 객체는 값이 바뀌면 특성도 바뀌기 때문에 값을 수정하면 사용하는 오브젝트는 그대로 둔 채로 변경 효과를 볼 수 있다.

### 정리
- 스프링이란?
    - 오브젝트가 어떻게 설계되고 만들어지고 관계를 맺는지 관심을 갖는 프레임워크
    - 객체지향으로 개발할 때 귀찮은 작업들을 도와주는 `도구`
    - 오브젝트를 어떻게 설계할지, 어떻게 분리할지, 개선할지, 의존관계를 가져갈지 결정하는 건 `개발자`
